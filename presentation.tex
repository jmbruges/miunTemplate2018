...
% aspect 43 or 169
\usepackage[tocsection]{beamerMiun2018}
...
\usepackage[siunitx,americancurrents]{circuitikz}

%\usetikzlibrary{babel}
\usepackage{tikz}
\usetikzlibrary{decorations.pathmorphing, decorations.pathreplacing, patterns}
\usetikzlibrary{arrows,snakes,shapes,angles}
...
%   \begin{frame}
%       \frametitle{Outline for my talk}
%       \tableofcontents
%   \end{frame}
    ...
    \begin{frame}[label=lists]{Lists and locales}
      ...
      \bigskip
      %\justifying
      {\uselanguage{swedish}Flygande bäckasiner söka hwila på mjuka tuvor.}\\
      {\uselanguage{german}Victor jagt zwölf Boxkämpfer quer über den großen Sylter Deich}\\
    ...
    \begin{frame}[label=columns]{Columns}
      ...
      \begin{column}{0.5\textwidth}  
           ...
           %\includegraphics[width=\textwidth]{resources/medipix}
           some text here some text here some text here some text here some text here
      \end{column}
      \end{columns}
    \end{frame}
    ...
    \begin{frame}[label=math]{Numerals and Mathematics}
      ...
      \begin{columns}[onlytextwidth]
        \column{.20\textwidth}
          1234567890
        \column{.20\textwidth}
          \firalining{1234567890}% this works only with FiraSans font
        \column{.20\textwidth}
      ...
      \begin{columns}[onlytextwidth]
        \column{.3\textwidth}
          $$\mathop{\int \!\!\! \int}_{\mathbf{x} \in \mathbb{R}^2}% \mathds{R}^2}
    ...
    \begin{frame}[label=figs1]{Figures}
        ...
        \begin{tikzpicture}[overlay,remember picture]
        	\node[anchor=south east,xshift=-25mm,yshift=20mm]
          at (current page.south east) {
            \includegraphics[width=50mm]{resources/medipix}
          };
      \end{tikzpicture}%
...
%     \end{frame}
    ...
    \begin{frame}[label=statediagr]{Tikz diagrams}
    \framesubtitle{State diagram}
    \begin{figure}
      \begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=4.5cm,
      thick,main node/.style={circle,draw,font=\sffamily\large\bfseries, text width=1cm, align=center, scale=0.7}]

      \node[main node] (A) {A Z=0};
      \node[main node] (B) [below of=A] {B Z=1};
      \node[main node] (C) [right of=B] {C Z=0};
      \node[main node] (D) [right of=A] {D Z=1};

      \path[every node/.style={font=\sffamily\small}]
      (A) edge [bend right] node[left] {x=1} (B)
          edge [loop above] node {x=0} (A)
      (B) edge node {x=0} (D)
          edge [bend right] node[below] {x=1} (C)
      (C) edge [bend right] node[] {x=0} (B)
          edge [bend right] node[right] {x=1} (D)
      (D) edge [bend right] node [above] {x=1} (A)
          edge [bend right] node[right] {x=0} (C);
      \draw (A) circle (0.8cm);
      \end{tikzpicture}
    ...
    % this can also be done with other highlighters: minted
